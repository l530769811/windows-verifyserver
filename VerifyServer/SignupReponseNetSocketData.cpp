...
CSignupReponseNetSocketData::CSignupReponseNetSocketData(int result)
	: m_result(result)
{
}


CSignupReponseNetSocketData::~CSignupReponseNetSocketData(void)
{
}

int CSignupReponseNetSocketData::_prepare_data(unsigned char* out_data, long len)
{
	int bret = false;
	...
	if(content!=0)
	{
		cJSON_AddStringToObject(content, JSON_COMMON_CTYPE_KEY, _T("user_regist_reponse"));
